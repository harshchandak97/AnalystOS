      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Available companies: {stocks.length}
          </p>
        </div>

        {/* Stock Cards - ALWAYS RENDER */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {stocks.map((stock, index) => {
            const stockId = stock.company_id.toLowerCase();
            const isSelected = selectedStocks.includes(stock.company_id) || selectedStocks.includes(stockId);
        ...
        {/* Show message if no stocks loaded yet */}
        {stocks.length === 0 && (
          <div className="text-center py-12 bg-white rounded-xl border-2 border-yellow-300">
            <p className="text-lg font-bold text-gray-900 mb-2">Waiting for company data...</p>
            <p className="text-sm text-gray-600">If this persists, try refreshing the page.</p>
          </div>
        )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">
                {selectedStocks.length} of {stocks.length}
              </p>
            ...
            <button
              ...
              className={`px-12 py-4 font-bold text-xl rounded-xl transition-all duration-200 shadow-lg ${
                selectedStocks.length > 0
